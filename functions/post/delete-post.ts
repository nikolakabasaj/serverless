...
export async function handler(event: any = {}): Promise<any> {
  ...
  if (!(await isPostMine(currentUserId, postId))) {
    return httpResponse(
      ...
      `User with id ${currentUserId} does not have a permission to delete post with id ${postId}`
    );
  }
  ...
  return httpResponse(
    HttpStatusCode.Ok,
    `Post with id ${postId} is successfully deleted`
  );
}

async function isPostMine(
