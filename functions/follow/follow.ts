...
export async function handler(event: any = {}) : Promise <any> {
  ...
  if (await isAllreadyFollowing(process.env.FOLLOW_TABLE_NAME!, currentUserId, followedUserId)) {
    return httpResponse(HttpStatusCode.Conflict, `User with id ${currentUserId} already follows user with id ${followedUserId}`);
  }

  const newFollow = new Follow(currentUserId, followedUserId);
  try {
    await db.add(process.env.FOLLOW_TABLE_NAME!, newFollow);
    return httpResponse(HttpStatusCode.Ok, `User with id ${followedUserId} successfully followed`);
  } catch (dbError) {
...
export async function isAllreadyFollowing(tableName: string, followerId: string, followedId: string) {
  ...
  const follow = await db.getByMultipleFields(tableName, Indexes.FollowerAndFollowedIdIndex, followerAndFollowedIdIndexFiels);
