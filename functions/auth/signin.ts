...
exports.handler = async function (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
	...
	if (!event.body) {
		return httpResponse(HttpStatusCode.BadRequest, 'You must provide a username and password');
	}
	...
	try {
		...
		return httpResponse(HttpStatusCode.Ok, 'Auth successfull', { token: token });
	} catch (err) {
